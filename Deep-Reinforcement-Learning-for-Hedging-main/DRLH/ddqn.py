import numpy as np
import torch
import matplotlib.pyplot as plt
from financial_models.asset_price_models import GBM
from financial_models.option_price_models import BSM, DigitCI
from machin.frame.algorithms import DQN
from torch import nn
import gym
import time
import gym_hedging

seed = 1234
np.random.seed(seed)
torch.manual_seed(seed)

tim = time.time()

mu = 0
dt = 1/128
T = 1
num_steps = T/dt
s_0 = 1
strike_price = s_0
sigma = 0.15
r = 0.01
final_coupon = 0.1

apm = GBM(mu=mu, dt=dt, s_0=s_0, sigma=sigma)
#opm = BSM(strike_price=strike_price, risk_free_interest_rate=r, volatility=sigma, T=T, dt=dt)
opm = DigitCI(strike_price=strike_price, risk_free_interest_rate=r, volatility=sigma, T=T, dt=dt, final_coupon=final_coupon)

env = gym.make('hedging-v0', option_price_model=opm, asset_price_model=apm, dt=dt, T=T, num_steps=num_steps, trading_cost_para=1,
                     L=1, strike_price=strike_price, int_holdings=False, initial_holding=0, mode="PL",
               )


num_actions = 21

class QNet(nn.Module):
    def __init__(self, state_dim, hidden_dim, action_num):
        super(QNet, self).__init__()

        self.fc1 = nn.Linear(state_dim, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, hidden_dim)
        self.fc3 = nn.Linear(hidden_dim, action_num)

    def forward(self, state):
        a = torch.relu(self.fc1(state))
        a = torch.relu(self.fc2(a))
        return self.fc3(a)

qnet = QNet(4, 20, num_actions)
qnet_t = QNet(4, 20, num_actions)

dqn = DQN(qnet, qnet_t,
          torch.optim.Adam,
          nn.MSELoss(reduction='sum'), discount=0.8, epsilon_decay=0.999, learning_rate=0.001,
          lr_scheduler=torch.optim.lr_scheduler.StepLR, lr_scheduler_kwargs=[{"step_size": 1000*128}])

num_eps = 1000
norm_factor = 10000

print(dqn)

def test_delta(n=10):
    rew = []
    for i in range(n):
        state = env.reset()
        done = False
        state = state[[0, 1, 2, 4]]
        while not done:
            action = state[3] - env.h
            print(state[3])
            print(env.h)
            new_state, reward, done, info = env.step(action)
            reward = np.sum(reward)
            new_state = new_state[[0, 1, 2, 4]]
            reward = -norm_factor*(reward**2) + 1 / 1000 * reward
            rew.append(reward)
            state = new_state
    return np.mean(rew)


def test(n=10):
    rew = []
    for i in range(n):
        state = env.reset()
        done = False
        state = state[[0, 1, 2, 4]]
        while not done:
            out = dqn.act_discrete({"state": torch.tensor(state, dtype=torch.float32).unsqueeze(0)})
            action = out.squeeze().detach().numpy() / num_actions
            new_state, reward, done, info = env.step(action - env.h)
            reward = np.sum(reward)
            new_state = new_state[[0, 1, 2, 4]]
            reward = -reward ** 2 + 1 / 1000 * reward
            #reward = -(action + env.h - state[3]) ** 2  # remove that
            if i == 1:
                print(action, state[3])
            rew.append(reward)
            state = new_state
    return np.mean(rew)

test_res_ddqn = []

rew = []
cost_l = []
rew_m_l = []
cost_m_l = []

for j in range(num_eps):
    print("episode: ", j)
    state = env.reset()
    done = False
    state = state[[0,1,2,4]]
    tmp_observations = []

    while not done:
        out = dqn.act_discrete_with_noise({"state": torch.tensor(state, dtype=torch.float32).unsqueeze(0)})
        action = out.squeeze().detach().numpy()/num_actions - env.h
        #print(action)
        new_state, reward, done, info = env.step(action)
        cost = reward[1]
        reward = np.sum(reward)
        new_state = new_state[[0, 1, 2, 4]]
        reward = -reward ** 2 + 1 / 1000 * reward
        #reward = -norm_factor*((reward) ** 2 + 1 / 1000 * reward)
        #print(reward)
        rew.append(reward)
        cost_l.append(cost)
        tmp_observations.append(
            {
                "state": {"state": torch.tensor(state, dtype=torch.float32).unsqueeze(0)},
                "action": {"action": out},
                "next_state": {"state": torch.tensor(new_state, dtype=torch.float32).unsqueeze(0)},
                "reward": float(reward),  # norm factor
                "terminal": done
            }
        )
        dqn.store_episode(tmp_observations)

        state = new_state
    
    rew_m_l.append(np.mean(rew))
    cost_m_l.append(np.mean(cost_l))

    if j % 20 == 0 and j != 0:
        print(test(10), test_delta(10))
        print("reward: ", np.mean(rew), np.mean(rew)/norm_factor)
        test_res_ddqn.append(np.mean(rew))
        rew = []
    

    if j > 100:
        for _ in range(int(num_steps)):
            dqn.update()

    if j % (num_eps/5) == 0:
        fig, ax1 = plt.subplots()
        ax2 = ax1.twinx()
        rew = []
        cost_l = []
        state = env.reset()
        done = False
        state = state[[0,1,2,4]]
        while not done:
            out = dqn.act_discrete({"state": torch.tensor(state, dtype=torch.float32).unsqueeze(0)})
            action = out.squeeze().detach().numpy()/num_actions - env.h
            new_state, reward, done, info = env.step(action)
            cost = -reward[1]
            reward = np.sum(reward)
            reward = -(reward) ** 2 + 1 / 1000 * reward
            new_state = new_state[[0, 1, 2, 4]]
            rew.append(reward)
            cost_l.append(cost)
            state = new_state

        ax1.plot([i for i in range(len(rew))], rew, label="Reward", color="green")
        ax2.plot([i for i in range(len(cost_l))], cost_l, label="Transaction Cost RH", color="red")

        ax1.set_xlabel('time')
        ax1.set_ylabel('Reward', color='g')
        ax2.set_ylabel('Cost', color='r')

        ax2.legend()
        plt.savefig("test_dqn_hedging_digital_CI_reward_cost" + str(j/num_eps*5) + ".png")
    

#dqn.save("dqn_model_1000")
plt.figure()
t = list(range(1, len(cost_m_l)+1))
plt.plot(t, rew_m_l, label="RL DQN Cost")
plt.xlabel("epoch times")
plt.ylabel("transaction cost")
plt.legend()
plt.savefig("DQN Hedging Digital CI Transaction Cost.png")

plt.show()
plt.clf()
plt.cla()
plt.close()

plt.figure()
t = list(range(1, len(rew_m_l)+1))
plt.plot(t, rew_m_l, label="RL DQN Reward")
plt.xlabel("epoch times")
plt.ylabel("reward")
plt.legend()
plt.savefig("DQN Hedging Digital CI Reward.png")

plt.show()
plt.clf()
plt.cla()
plt.close()

print("__________")
print(rew_m_l)
print(np.mean(rew_m_l), np.std(rew_m_l))
print("__________")
print(cost_m_l)
print(np.mean(cost_m_l), np.std(cost_m_l))

print(time.time()-tim)

