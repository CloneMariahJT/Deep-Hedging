...
class HedgingEnv(gym.Env):
    ...
    def __init__(self, option_price_model, asset_price_model, dt, T, num_steps=100, trading_cost_para=0.01,
    ...
    def _compute_cf_reward(self, new_h, next_price, delta_h):
        ...
        if self.done:
            ...
            if self.option_price_model.option_type() == "BSM":
                payoff = self.L * max(0, next_price - self.strike_price)
            elif self.option_price_model.option_type() == "DigitCI":
                payoff = self.L * (next_price > self.strike_price) * self.option_price_model.coupon
    ...
    def step(self, delta_h):
        ...
        if self.n == self.num_steps:
            self.done = True
        if self.mode == "CF":
            reward = self._compute_cf_reward(new_h, next_price, delta_h,self.asset_price_model)
        elif self.mode == "PL":
