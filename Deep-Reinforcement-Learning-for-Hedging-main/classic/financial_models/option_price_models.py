...
class DigitCI(GenericOptionPriceModel):
    def __init__(self, strike_price, risk_free_interest_rate, volatility, T, dt, final_coupon):
        self.strike_price = strike_price
        self.risk_free_interest_rate = risk_free_interest_rate
        self.volatility = volatility
        self.T = T
        self.dt = dt
        self.coupon = final_coupon
    ...
    def compute_option_price(self, n, asset_price, mode="step"):
        ...
        d_1 = (np.log(asset_price / self.strike_price) + (self.risk_free_interest_rate + self.volatility**2 / 2)
               * time_to_maturity) / (self.volatility * np.sqrt(time_to_maturity))
        d_2 = d_1 - self.volatility * np.sqrt(time_to_maturity)
        PVCP = self.coupon * np.exp(-self.risk_free_interest_rate * time_to_maturity)
        option_price = norm.cdf(d_2) * PVCP
        return option_price
    ...
    def compute_delta_ttm(self, ttm, asset_price):
        ...
        d_1 = (np.log(asset_price / self.strike_price) + (self.risk_free_interest_rate + self.volatility**2 / 2)
               * time_to_maturity) / (self.volatility * np.sqrt(time_to_maturity))
        d_2 = d_1 - self.volatility * np.sqrt(time_to_maturity)
        delta = self.coupon * np.exp(-self.risk_free_interest_rate * time_to_maturity) * norm.pdf(d_2) / (self.volatility * asset_price * np.sqrt(time_to_maturity))
        return delta
